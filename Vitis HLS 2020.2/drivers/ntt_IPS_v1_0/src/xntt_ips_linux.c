...
typedef struct {
    ...
    XNtt_ips_uio_map maps[ MAX_UIO_MAPS ];
} XNtt_ips_uio_info;

/***************** Variable Definitions **************************************/
static XNtt_ips_uio_info uio_info;
...
static int uio_info_read_name(XNtt_ips_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/name", info->uio_num);
    return line_from_file(file, info->name);
}

static int uio_info_read_version(XNtt_ips_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/version", info->uio_num);
    return line_from_file(file, info->version);
}

static int uio_info_read_map_addr(XNtt_ips_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
static int uio_info_read_map_size(XNtt_ips_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
int XNtt_ips_Initialize(XNtt_ips *InstancePtr, const char* InstanceName) {
	XNtt_ips_uio_info *InfoPtr = &uio_info;
